...
export class FriendsService {
  ...
  async requestFriend(friendRelationDto: FriendRelationDto): Promise<void> {
    ...
    const relation = await this.friendsRepository.findFriendRequest(
      senderId,
      receiverId,
    );
    if (relation) {
    ...
    const reverseRelation = await this.friendsRepository.findFriendRequest(
      receiverId,
      senderId,
    );
    if (reverseRelation) {
  ...
  async cancelFriendRequest(
    friendRelationDto: FriendRelationDto,
  ): Promise<void> {
  ...
  async deleteFriendRelation(userId: number, friendId: number) {
    ...
    const friendRelation = await this.friendsRepository.findRelation(
      userId,
      friendId,
    );
    if (!friendRelation) {
  ...
  async searchFriend(
    ...
    return friends.filter((friend) => friend.name.includes(name));
  }
